...
const _pLimit = _interopRequireDefault(require('p-limit'));
...
function processPattern(globalRef, pattern) {
  ...
  return (0, _globby.default)(pattern.glob, globOptions).then((paths) =>
    Promise.all(
      paths.map((from) =>
        limit(() => {
          const file = {
          ...
          file.relativeFrom = _path.default.relative(
            pattern.context,
            file.absoluteFrom
          );
