...
const _loaderUtils = _interopRequireDefault(require('loader-utils'));

const _cacache = _interopRequireDefault(require('cacache'));

const _serializeJavascript = _interopRequireDefault(
  require('serialize-javascript')
);

const _findCacheDir = _interopRequireDefault(require('find-cache-dir'));
...
function postProcessPattern(globalRef, pattern, file) {
  ...
  return getStats.then((stats) => {
    ...
    return (0, _promisify.readFile)(inputFileSystem, file.absoluteFrom)
      .then((content) => {
        if (pattern.transform) {
          ...
          if (pattern.cache) {
            ...
            const cacheKey = pattern.cache.key
              ? pattern.cache.key
              : (0, _serializeJavascript.default)({
                  name: _package.name,
                  version: _package.version,
                  pattern,
                  hash: _crypto.default
                    .createHash('md4')
                    .update(content)
                    .digest('hex'),
                });
            return _cacache.default.get(globalRef.cacheDir, cacheKey).then(
              (result) => {
                logger.debug(
              ...
              () =>
                Promise.resolve()
                  .then(() => transform(content, file.absoluteFrom)) // eslint-disable-next-line no-shadow
                  .then((content) => {
                    logger.debug(
                      `caching transformation for '${file.absoluteFrom}'`
                    );
                    return _cacache.default
